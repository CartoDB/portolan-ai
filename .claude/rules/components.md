...
- **MentionChips** (`mention-chips.tsx`): Shared component rendering `@type:id` mentions as colored pill chips above the textarea. `@panel` (green) for dashboard panels. Click chip to remove. Used in the explore (`/:slug`) input. Extracts mentions via `/@(\w+):(\S+)/g` regex.
