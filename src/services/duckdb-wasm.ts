  ...
  initPromise = (async () => {
    ...
    for (let attempt = 1; attempt <= MAX_INIT_RETRIES; attempt++) {
      try {
        ...
        for (const ext of ["httpfs", "spatial"]) {
          try {
        ...
        try {
          ...
          const expected = ["httpfs", "spatial", "iceberg"];
...
function wrapSqlForGeometry(
  ...
  // When a native CRS is known, reproject to EPSG:4326 for display (no-op for 4326/undefined).
  const rawGeomExpr = isNativeGeometry ? `"${geomColumn}"` : `ST_GeomFromWKB("${geomColumn}")`;
  const geomExpr = transform4326Expr(rawGeomExpr, nativeCrs);
  ...
  // When a native CRS was supplied, the WKB blob has been turned into a (reprojected) geometry
  // and must be re-serialized with ST_AsWKB. Without a nativeCrs this reduces to the original behavior.
  const wkbExpr = isNativeGeometry || nativeCrs ? `ST_AsWKB(${geomExpr})` : `"${geomColumn}"`;
...
/**
 * Run a SQL query and return ALL rows as plain JS objects.
 *
 * Low-level helper for internal callers (e.g. catalog index loader) that need
 * the full result set, not the capped sample runQuery returns. Reuses the same
 * DuckDB singleton (initDuckDB) and the same robust Arrow→JS conversion
 * (arrowToJs, handles BigInt/Struct/List) as runQuery, and follows the same
 * connection lifecycle (connect, then close in finally).
 */
...
export async function runQuery(input: { sql: string; nativeCrs?: string } | string): Promise<{
  ...
  try {
    ...
    const finalSql = geomColumn
      ? wrapSqlForGeometry(sql, geomColumn, geomInfo?.allColumns ?? [], isNativeGeometry, nativeCrs)
      : sql;
